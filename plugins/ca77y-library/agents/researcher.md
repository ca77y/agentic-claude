...
- Hand it that set directly: never rescan `library/raw/` or re-derive the list from the vault. Name any collected path the write leaves un-indexed in your report rather than reporting the run complete over it, taken from the full-ingest scribe's returned indexed/un-indexed accounting.
