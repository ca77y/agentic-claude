---
name: bootstrap
description: Create, complete, or explicitly repair a project's local board and forge declarations (`docs/BOARD.md`, `docs/FORGE.md`) from project facts and user choices. Use for one-time engineering setup, not ordinary implementation or external board/forge operations.
---
...
On every invocation, open or create the run's durable ledger using the ledger procedure at `${CLAUDE_PLUGIN_ROOT}/skills/deliver/references/ledger.md` and the template at `${CLAUDE_PLUGIN_ROOT}/skills/deliver/assets/ledger.md` before production or delegation. The main agent is its sole writer, including for trivial work and runs without subagents. Record returned agent IDs, assignments and progress; update before dispatch and waits, immediately after dispatch, on results and gate/failure changes, and before handoff or the final response. Reuse the ledger on resume and across entry points within the same run, preserving its failure history. A separate later user request gets a new run ledger; prior run history remains context, not its attempt count. Link the ledger in the final response.

Read `${CLAUDE_SKILL_DIR}/references/board.md` only when preparing the board declaration and `${CLAUDE_SKILL_DIR}/references/forge.md` only for the forge declaration. Use their bundled templates with repository facts and supplied choices. A project may explicitly choose no board or no forge; record that instead of inventing integrations.

The main agent may draft directly or use `slop-eng:writer`. A fresh `slop-eng:auditor` validates the setup spec and a different fresh auditor validates the generated declarations. Give each the explicit bootstrap scope so missing output declarations are not mistaken for missing production prerequisites.
