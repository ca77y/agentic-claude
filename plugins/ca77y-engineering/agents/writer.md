...
**Spec pass:** the spec's file path; the acceptance criteria it was written against; the `auditor` gate status (passed, rerun after edits, or blocked and escalated) and what the findings changed; and any scope question the `lead` should settle.
