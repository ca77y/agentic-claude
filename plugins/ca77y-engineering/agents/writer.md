...
**Enumerating a definition file's edit sites includes its frontmatter `description`, in that same pass.** Where the task changes behaviour the `description` also states, choose one outcome at authoring time — never leave it to the `coder`: the `description` joins the enumerated edit sites inside the `coder`'s scope, or its owning mechanism is named per the rule above with a Tasks entry marked not the `coder`'s. Where it states nothing the change falsifies, record that it was checked and needs no change. Governs any definition file with a frontmatter `description` — agent or skill.
