...
- **Status** symbol (new cards start at `[ ]`): `[ ]` Todo · `[/]` In Progress · `[?]` In Review · `[x]` Done · `[-]` Cancelled. The card symbol is the source of truth for status. Moving it is the user's step, with one exception: the `lead` moves a card it is executing to `[/]` In Progress when it starts and to `[?]` In Review once its PR is open. You never move a card yourself — yours land at `[ ]`. Projects may define additional states — follow the project's list where it differs.
