...
**Read the tracking declaration before you touch the board.** How this project tracks work is declared, never assumed: read `docs/BOARD.md`, at that fixed path, before you search or create anything. It records which board holds the project's cards, the concrete call bound to each operation (locate, read, search, create, transition, plus comment and update where the project authorises them), the card shape and its status vocabulary, and which operations the pipeline is permitted to write. Reach the board only through those bindings, and stay inside that write authority. A board can be Markdown files committed in this repository, a hosted tracker behind an MCP server or a CLI, or nothing at all — your job is identical across all three, because everything board-shaped comes out of the declaration. There is no skill to invoke for this: reading the declaration is a file read you do yourself.
...
2. **Read project context.**
   ...
   - Existing cards, through the declaration's `search` and `read` bindings, to avoid duplicates and find the right dependencies; settled capability docs and in-flight specs.
...
- **Re-query the board** through the declaration's `search` binding — including whatever holds parked and completed work, which on some boards is a separate folder and on others a status filter — and re-check the dependency edges. Reconcile against anything that appeared since intake: fold, supersede, or cross-link.
...
- **Priority** when known, on the board's own scale. **Identity**: a stable, unique id — lowercase kebab-case where the board does not impose a format — reused for the story's spec file and, by whatever rule the project's forge declaration records, for its branch — so the same story is one name across the board, the repo, and the PR. Dependents point back at it through the board's own dependency or blocking link.
...
- **Honor the format quirks the declaration records.** A board whose view is built by scanning files for checkbox markers, for instance, surfaces *every* match as a separate item — so on that board nested checkboxes create phantom cards and context belongs on plain bullets. Quirks like that are facts about the board you are writing to, recorded in the declaration; they are not rules to carry to the next one.
