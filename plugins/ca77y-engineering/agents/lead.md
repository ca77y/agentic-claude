...
10. **Integration review and story acceptance.** Have the `reviewer` review the integrated result — the **committed** story range (engineers commit their units, so post-merge there is no uncommitted diff; tell the `reviewer` the `story-base..story-head` range to review) — on two axes:
   ...
   **Route each finding to the owning `engineer`** — never to a coder directly; the engineer owns its fix. For a single unit, route to that engineer. Re-integrate and re-review, capped at 2–3 rounds, then escalate what remains. If an engineer reported a **blocked review gate** (its unit review returned no result), that unit is *not* reviewed: re-run a unit review over its diff via the `reviewer` before shipping, or hold the story and escalate — never ship it as reviewed. Do not open the PR while a story acceptance criterion is unmet.
...
13. **Fire the external PR review.** Once the PR is open, dispatch `gemini` in **pr-review** mode with just the PR link — this is fire-and-forget: `gemini` launches `agy` in the background and returns immediately, and `agy` posts its review as comments directly on the PR. Do not wait for it, do not poll for a result, and do not let it hold up any of the steps above or your final handoff — it runs independently of the story's acceptance gate.
...
- `auditor` — spec validation and spec-set integration review (readiness gates). A genuine no-result blocks the gate.
- `reviewer` — integration review of the committed story diff.
- `writer` — the single docs + spec-conversion pass after integration.
- `gemini` (pr-review mode) — fires the independent `agy` PR review once the PR opens; fire-and-forget, nothing to relay.
...
- Do not dispatch a spec the auditor has not validated; when the story is split, do not dispatch to engineers until the spec set passes its integration review.
