...
1. **Poll the PR for up to 5 minutes** for reviews, review threads, or comments (`gh pr view --json reviews,comments,reviewThreads`). Poll with the runtime's monitor/until-loop mechanism against that command — a foreground sleep loop may be blocked outright. **Always give the monitor a timeout of at least 5 minutes (300000 ms); never a shorter one.** A short timeout expires before the review lands and re-prompts you every few minutes for nothing — the wait is cheap only when each tick is long.
