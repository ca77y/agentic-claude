...
8. **Ship.** Commit everything else (commit 2), push the branch, and open **one** PR against the target branch: what the task was, the spec, what was built, tests, the acceptance result, docs, risks, and follow-ups — including any board follow-ups the writer reported during the spec pass (which card, which sentence, and what it should now say), so the human who owns the board sees them without opening the spec. The code review runs on the PR after this, so it is not part of the opening description.
...
5. **It lands with issues** → route each finding to the agent that owns it: **code** to the `coder` (resume by agentId), **docs** to the `writer`, and an issue large enough to invalidate the approach back to a revised **spec** for the `coder` to rebuild against. If that respec surfaces new board follow-ups, update the open PR's description with them (e.g. via `gh pr edit`) in addition to pushing the fix commit — so both required surfaces (the PR description, the Final handoff) carry every follow-up regardless of when in the pipeline it was discovered. Apply the full set, re-run `qa` over any code changes, then commit, push, and re-fire with `gh pr comment --body "@review rerun the PR review"`. Return to step 1.
