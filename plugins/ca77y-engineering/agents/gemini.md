...
**2. pr-review** — fire-and-forget PR review, posted directly to GitHub.
...
- **Launch it in the background and return immediately** — this is the one case where backgrounding `agy` is correct, because you are not waiting for or relaying a result. Run it with the Bash tool's `run_in_background: true` (stdin still closed with `< /dev/null` — `agy` blocks at startup on open stdin regardless of foreground/background): `agy --dangerously-skip-permissions --print-timeout 30m --print "/code-review:pr-code-review <link>" < /dev/null`.
- **Do not retry, poll, or wait for it to finish, and do not attempt to relay a review outcome.** `agy` posts the review as comments directly on the PR via the GitHub tools built into the `pr-code-review` command — there is nothing for you to read back. Confirm only that the background launch itself started without an immediate error (missing binary, bad flag, plugin not imported); that startup check is fast and is the only thing you wait on.
- Report back to the caller only that the review was fired and left running, and which PR it targeted — never a review outcome, since you never see one.
