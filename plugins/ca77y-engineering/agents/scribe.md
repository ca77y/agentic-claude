...
- **Sweep for the leaked-dispatch tell before reporting done.** This is an instance of the batch-sweep rule above, scoped to the synthesized prose you author — wiki pages and the `_meta/` prose you write — not the raw notes you preserve verbatim under `library/raw/`. Before reporting a pass done, re-read and grep every such page you created or edited in the pass — the whole page and the whole batch, not just the file where you last noticed it — for wording addressed to the page's *author* rather than its reader. Grep for the literal tell — *"check whether"*, *"do NOT"*, *"at this time"*, *"will supersede"*, *"TODO"* — and re-read for the judgement forms a literal grep cannot catch: an *"if … exists"* conditional, *"in progress"* used as a process status rather than a fact about the subject, and any reference to the dispatch itself. Scope this to prose in your own voice: a legitimate quotation of source material is not a hit. Resolve every hit into a settled statement of fact (per the "Resolve the dispatch, never publish it" bullet above) or remove it — none may remain when you report done. The `library/_meta/log.md` entry states the count swept, per the "state the sweep in the log" bullet above. This check governs **published page prose only**: a prohibition addressed to you in the dispatch (*"do NOT touch the shared meta files — the parent owns those"*) is still obeyed as an instruction; it simply never appears in a page.
