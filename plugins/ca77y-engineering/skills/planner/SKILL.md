---
name: planner
description: Interactive planning workflow that records an approved story as a local board card and, when the work warrants it, writes its spec under docs/specs/. Use when the user has a shaped idea, an approved story, or an existing card and wants it captured in docs/tasks/ — and a goal/design/requirements/scenarios/tasks spec prepared when the work is large enough to need one.
---
...
Turn an approved, shaped story into tracked work. Planner owns two artifacts:

1. **Story card** — always. One checkbox in the story's board file under `docs/tasks/`, in the Obsidian Tasks format. This is the unit of work tracking.
2. **Spec** — when the work warrants one. One file at `docs/specs/<slug>.md` (`goal -> design -> requirements/scenarios -> tasks`). Small, self-contained stories ship from the card alone with no spec.

There is no epic/child hierarchy: one story is one card, scoped as a substantial, self-contained chunk of work. Do not split a story into multiple tracked cards.
...
Each story lives in its own board file under `docs/tasks/<slug>.md`, holding a single Tasks-format checkbox — the story card. The slug is the stable id used for the branch (`feat/<slug>`) and the spec filename (`docs/specs/<slug>.md`).
...
- **Id** is `🆔 <slug>` (lowercase kebab-case); another story depends on it via `⛔ <slug>`.
- Put planner-ready context (background, scope, acceptance criteria, references) in the card's body lines or, when the work warrants one, in the spec it links to. Do not paste long research into the card; link `docs/library/`, docs, and code paths.
...
- `#marketing` / `#support` only when the work is primarily non-product-implementation work; if such work needs product changes, record a separate `#feature`/`#improvement`/`#bug` story.

Stories may depend on one another via native Tasks dependencies (`🆔`/`⛔`) for sequencing — but a single story is never broken into child cards. If approved work depends on a prerequisite that isn't yet a story, that prerequisite is its own story with its own card and slug.

## When to also write a spec

Write a spec when the work is large or uncertain enough that the engineer benefits from a contract: multiple requirements, non-trivial design or data-flow decisions, several scenarios, cross-area changes, migrations, or real regression risk. Most substantial stories warrant one. Skip the spec for small, self-contained changes whose acceptance criteria fit cleanly in the card. When in doubt, ask the user. When a spec exists, link it from the card body (`[spec](../specs/<slug>.md)`).

## Workflow

1. Load context: root and area `CLAUDE.md`/`AGENTS.md`; `docs/tasks/CLAUDE.md`; `docs/specs/README.md`; the shaped story or existing card; related docs and code — especially settled capability specs in `docs/features/` and in-flight specs in `docs/specs/`.
...
3. Resolve the slug and target board file:
   ...
   - The story's board file is `docs/tasks/<slug>.md`. Avoid duplicate cards — update the existing card instead.
4. Draft or update the card with exactly one type tag, status `[ ]`, priority, `🆔 <slug>`, and any `⛔` dependencies on other stories. Keep scope and acceptance criteria observable.
...
8. Finish only when the user approves:
   - Write/update the card in `docs/tasks/<slug>.md`, and the spec in `docs/specs/` when one was warranted.
...
Use the template for the chosen primary type to shape card body / spec content; load only the one you need from `${CLAUDE_PLUGIN_ROOT}/references/templates/` (`feature.md`, `improvement.md`, `bug.md`). The repo may provide stronger Templater scaffolds under `docs/_templates/`; prefer those when present.
...
- Do not split a story into multiple tracked cards. One story, one card.
- Do not silently change scope; call out significant changes and ask for approval.
- Do not write a spec for `research`, `marketing`, or `support` work until it is refined into a concrete `#feature`/`#improvement`/`#bug` story.
