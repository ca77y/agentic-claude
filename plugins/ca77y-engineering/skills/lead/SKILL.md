...
2. **Create the workspace** — all of it before any dispatch; everything from here happens in the worktree.
   ...
   - If the harness refuses every file write until the session is isolated (a background job in particular), enter the worktree you just created with `EnterWorktree` **by `path`, never by `name`** — read `references/recovery.md`. Nothing moves; every dispatch still names the absolute path.
...
3. **Spec.** Dispatch `ca77y-engineering:writer` to author the spec in the project's specs area — its *Acceptance criteria* section transcribes the card's `## Acceptance criteria` verbatim, `AC1`…`ACn`, taken **after** any criterion correction the spec pass makes on the card — and record its agentId if the dispatch produced one. Then, in this order:
   ...
   - **The floor.** Immediately after, before dispatching the `coder`, run the project's lint command once (per *Running a project command*). Once trusted and failing: a failure naming **any path commit 1 landed** is this run's — route it to the `writer` per *When a gate finds a problem* and commit the fix as the spec-format-fix commit before dispatching the `coder`; a failure naming **only paths outside commit 1** is pre-existing — record and relay it, never route, fix, or stop on it. The floor runs once per run, not per round.
...
7. **Docs.** Dispatch `ca77y-engineering:writer` for the docs pass — a fresh dispatch, either mode: durable docs for what shipped, the spec converted into its permanent home and removed from the specs area. Hand it the **spec commit** and the **round commit references** from the ledger — it diffs the spec commit against `HEAD` to establish what shipped, since a gate finding can have changed the design. Trust what it returns; there is no docs gate.
...
- **`--fast` is the user's flag, never your judgement**, and steps only the model, per *The `--fast` flag*.
