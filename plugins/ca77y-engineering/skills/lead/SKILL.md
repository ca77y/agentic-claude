...
**`--fast`.** When `--fast` is on the invocation, read `${CLAUDE_SKILL_DIR}/references/fast.md` before the first dispatch and follow it as part of these instructions; it covers the one thing the flag changes — the model each dispatch runs on, one tier down — the table that resolves it per dispatch, and everything it leaves alone. Two facts hold whether or not the flag is present: without it, omit `model:` from every `Agent` call — each agent runs on its own frontmatter pin; and **never turn it on or off yourself**.
