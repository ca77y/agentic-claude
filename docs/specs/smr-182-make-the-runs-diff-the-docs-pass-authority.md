...
- [ ] Read `plugins/ca77y-engineering/agents/writer.md`'s `## Docs pass` section end to end, including `### Reconciling what you touch` and `### Checking your own output`, so the new subsection composes with them rather than overlapping them.
- [ ] Add `### What shipped is the run's diff, not the spec` under `## Docs pass`, positioned **before** `### Reconciling what you touch`, carrying: the designed divergence (AC1); its cause (AC2); the two git reads and what the round-commit messages supply (AC3, AC4); per-claim reconciliation (AC5); the diff's authority, stated as authority (AC6); the spec's retained role as the source of intent, reconciled with the previous sentence (AC7); that divergence is reported (AC8); and the unobtainable-references outcome, including the ban on reporting the spec as reconciled (AC10).
- [ ] Insert the new numbered step before the authoring step in the `## Docs pass` list — it establishes what shipped and points at the subsection — and renumber the steps that follow (the list goes from 8 steps to 9).
- [ ] Make the spec-conversion step point at the same subsection for the reconcile-against-the-diff duty rather than restating it (AC11).
- [ ] Add the divergence line to the **Docs pass** bullet of `## Final report` in `writer.md` (AC8).
- [ ] Re-read `writer.md` end to end and confirm the duty is stated exactly once, with pointers from the steps that need it and no second independently readable copy (AC11).
- [ ] Edit step 7 (*Docs*) of `## Workflow` in `plugins/ca77y-engineering/skills/lead/SKILL.md` to hand over the spec commit and the round commit references from the ledger, with the one-clause reason, and no dispatch-mode requirement (AC9).
- [ ] Update the **Docs pass** bullet under `### writer — the task's spec, then its docs` in the root `README.md` (AC12).
- [ ] Update the `7. **Docs**` bullet of the lead's numbered workflow in the root `README.md` (the addition recorded in *Deviations from the card*).
- [ ] Confirm the `Docs` row of the README's pipeline-at-a-glance table (`durable docs (format/lint self-checked); spec converted & removed`) still reads true; leave it unchanged if it does, and record that it was checked.
- [ ] Confirm `writer.md`'s and `SKILL.md`'s frontmatter `description` values are byte-identical to `HEAD = 2b564ed` — the disposition recorded in the Boundary is *checked, no change needed*, so a diff in either is a defect to undo.
- [ ] Run the Validation list above, items 1–7, and record each outcome.
