...
- **`docs/AGENTS_IMPROVEMENTS.md`** — append-only, and **no entry count appears anywhere in this spec**, deliberately: a count stated in a file this same pass appends to invalidated itself twice in this run's gate rounds. What the build must do is bounded by property, not arithmetic: **change no existing entry**, and leave in place the entry titled *"An enumeration in a Validation item has to come from the item's own command"*, which this delta does not convert. An entry this pass drafted and then **withdrew in the same pass**, once the scope change absorbed its point, was never committed and so never entered the log — nothing was converted and no commit trace is owed for it (see *Deviations from the card*). The build adds an entry only if it hits fresh friction of its own; it does not re-file the `writer.md:52` owning-mechanism gap, which is already logged.
