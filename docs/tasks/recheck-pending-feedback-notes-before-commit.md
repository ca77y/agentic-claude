...
- [<] Re-check pending pipeline-feedback notes against the diff that ships them #improvement 🔽 🆔 recheck-pending-feedback-notes-before-commit
  ...
  - Also harmonize that paragraph's root-checkout wording while editing it. It asserts "the repository root checkout is off-limits" across ten agent files, which was unambiguous until [`install-dependencies-in-story-worktrees`](install-dependencies-in-story-worktrees.md) made the root checkout deliberately **readable** for dependency and vendor sources in the five files the two paragraphs overlap. It is not a contradiction — here "off-limits" is scoped to *writing* `AGENTS_IMPROVEMENTS.md` — but a reader meeting both paragraphs has to work that out. Word it as off-limits **for writes**.
  - Acceptance criteria:
  ...
  - The shared paragraph's root-checkout sentence says off-limits for **writes**, so it no longer reads as contradicting the read/write distinction the worktree-addressing paragraph now draws.
