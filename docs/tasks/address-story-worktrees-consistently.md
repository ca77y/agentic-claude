...
- [ ] Give agents one convention for addressing a story worktree #improvement ⏫ 🆔 address-story-worktrees-consistently
  ...
  - Scope: the shared worktree paragraph every dispatched agent carries (`lead`, `reviewer`, `coder`, `writer`, `auditor`), and the worktree convention in the root [`CLAUDE.md`](../../CLAUDE.md). Pick one resolution rather than both.
  - Acceptance criteria:
