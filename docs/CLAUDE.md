...
- Record a problem in `issues/` only when it is real but no solution could be identified on our side. State what was investigated, the evidence, and what would unblock it. Once a fix becomes identifiable, replace the note with an issue on the board (see [`BOARD.md`](./BOARD.md)).
